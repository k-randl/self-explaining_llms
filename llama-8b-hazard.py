...
for step, s in enumerate(tqdm(data_test.values)):
    ...
    # counterfactual:
    chat, _, _ = pipe.generate("Provide a version of the announcement that would alter your assessment while changing as few words in the original announcement as possible. Make sure to answer with only the new version.", output_ids)
    result['chat'].extend(chat[2:])
