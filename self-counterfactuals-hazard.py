...
path = f'results/food incidents - hazard/{SAVE_PREFIX}{MODEL_NAME}-self-counterfactuals.pkl'
...
with no_explain(pipe, reset_on_generate=True):
    for step, s in enumerate(tqdm(data_test.values)):
        ...
        # create prompt:
        prompt, label, spans = loader.createPrompt(tuple(s))
        ...
        for l, p in label_probs:
            ...
            for _ in range(NUM_SAMPLES*2):
                ...
                chat, _, _ = pipe.generate(f"Provide a version of the announcement that would alter your assessment to \"{l}\" while changing as few words in the original announcement as possible. Make sure to only answer with the changed announcement.", output_ids, do_sample=True, temperature=TEMPERATURE)
